...
import ProductDetailsGeneric from './productDetailsGeneric.component';
...
describe('<ProductDetailsGeneric />', () => {
  beforeEach(() => {
    store = mockStore(initialState);
  });
  it('renders <ProductDetailsGeneric /> component', () => {
    const wrapper = shallow(<ProductDetailsGeneric isCSR={false} store={store} api={JSONData} />).dive();
    expect(wrapper).to.have.length(1);
  });

  it('test with mock productItem', () => {
    const wrapper = shallow(<ProductDetailsGeneric isCSR={false} store={store} api={JSONData} productItem={productItem} />).dive();
    expect(wrapper).to.have.length(1);
  });
  ...
  it('test with full mount', () => {
    const wrapper = mount(
      <Provider store={store}>
        <ProductDetailsGeneric isCSR={false} api={JSONData} productItem={productItem} />
      </Provider>
    );
    expect(wrapper).to.have.length(1);
  });
  ...
  it('test with client side rendering', () => {
    const wrapper = shallow(<ProductDetailsGeneric store={store} productItem={productItem} />).dive();
    expect(wrapper).to.have.length(1);
  });

  it('test with getInitialProps method cmsPageInfo', () => {
    ProductDetailsGeneric.getInitialProps({ cmsPageInfo: { previewId: 3678641 }, env: { API_HOSTNAME: 'http://35.188.171.242/' } }).then(response => {
  ...
  it('test with getInitialProps method pageInfo', () => {
    ...
    ProductDetailsGeneric.getInitialProps({ pageInfo, env: { API_HOSTNAME: 'http://35.188.171.242/' } }).then(response => {
      console.log('response received..', response);
    });
    const wrapper = shallow(<ProductDetailsGeneric store={store} pageInfo={pageInfo} productItem={productItem} />).dive();
    expect(wrapper).to.have.length(1);
  });

  it('test with actions', () => {
    const { breadCrumb, name } = productItem;
    const wrapper = shallow(<ProductDetailsGeneric store={store} isCSR={false} api={JSONData} productItem={productItem} />).dive();
    wrapper
      .find(ProductActionItems)
      .props()
      .onClickIncrementQuantityLogGA({ breadCrumb, name });
    wrapper
      .find(ProductActionItems)
      .props()
      .onClickDecrementQuantityLogGA({ breadCrumb, name });
    expect(wrapper).to.have.length(1);
  });
  ...
  it('test mixedMedia meta data', () => {
    ...
    const wrapper = shallow(<ProductDetailsGeneric isCSR={false} productItem={mixedMediaProductItem} store={store} api={JSONData} />).dive();
    expect(wrapper).to.have.length(1);
  });
});
